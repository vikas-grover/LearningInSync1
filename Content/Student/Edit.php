...
<?php
	...
	$update_code =array(1);	unset($update_code);
	...
	foreach ($_POST['toupdateToLName'] as $name)  {
		...
		$x =0;
	foreach ($_POST['toupdateToSC'] as $code)  {
		if (!$code=="") {
			$update_code[$x] =$code;
			$x++;
		}//store array of new country names
	...
	for($x=0;$x<$countUpdates;$x++)  {
	$query = "Update student Set first_Name = '$update_fname[$x]', last_Name = '$update_lname[$x]', school_id = '$update_code[$x]' WHERE id = '$student_id[$x]'";
     echo $query.'<br />';
  ...
  while ($row = mysqli_fetch_array($result)) {
	...
	echo 'School ID: '. $row['school_id'];
	echo '<input type="text" id="school_id" name="toupdateToSC[]" />';
	echo '<br />';
	}// close the while
	mysqli_close($dbc); 
?>
